...
public class ChartSlice {
	public Hashtable trainSlices = new Hashtable();
	Chart chart;
	...
	public void makeStationSlice(Station station) {
		//获取事件
		Vector events = getStationEventsOfStation(station);
	...
	//获取车次切片的同向事件：让、越
	public Vector getTrainEventsOfTrainSameDir(Train train) {
		Vector events = new Vector();
		TrainSlice trainSlice = (TrainSlice) trainSlices.get(train);
		
		Enumeration en = trainSlices.keys();
	...
	//获取车次切片的反向事件：会
	public Vector getTrainEventsOfTrainDiffDir(Train train) {
		Vector events = new Vector();
		TrainSlice trainSlice = (TrainSlice) trainSlices.get(train);
	...
	//获取车次切片的车站事件：通、到、发
	public Vector getStationEventsOfTrain(Train train) {
		TrainSlice trainSlice = (TrainSlice) trainSlices.get(train);
		Vector events = (trainSlice == null) ? new Vector() : trainSlice.getStationEvents();
	...
	//获取车站切片事件
	private Vector getStationEventsOfStation(Station station) {
		Vector events = new Vector();
