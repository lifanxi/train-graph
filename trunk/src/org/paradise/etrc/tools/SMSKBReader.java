...
//			System.out.println(trainName);
	...
	public void dumpAll(String path) throws IOException {
		File fTK = new File(path + "smtk.csv");
		File fCC = new File(path + "smcc.csv");
		File fID = new File(path + "smid.csv");
		
		BufferedWriter outTK = new BufferedWriter(new FileWriter(fTK));
		BufferedWriter outCC = new BufferedWriter(new FileWriter(fCC));
		
		outTK.write("车次,停靠站,到达,出发,里程");
		outTK.newLine();
		...
		for(int trainIndex = 0; trainIndex < cc.size(); trainIndex++) {
			...
			for(int i=startIndex; i<=endIndex; i++) {
				...
				String str_arrive = (String)tkInfo[2];
				String str_leave = (String)tkInfo[3];
				String dist = tkInfo[1];
				
				outTK.write(trainInfo[0] + "," + tkName + "," + str_arrive + "," + str_leave + "," + dist);
				//System.out.println(trainInfo[0] + "," + tkName + "," + str_arrive + "," + str_leave);
				outTK.newLine();
			}
		}
		...
		BufferedWriter outID = new BufferedWriter(new FileWriter(fID));
		...
		while(en.hasMoreElements()) {
			String cc1 = (String) en.nextElement();
			String ccID = (String)ccIndex.get(cc1);
			String ccAll = ((String[])cc.get(Integer.parseInt(ccID)))[0];
			outID.write(cc1 + "," + ccAll);
			outID.newLine();
		}
...
//			for(int i=0; i<sm.cctk.size(); i++) {
////				System.out.println(i);
//				String tkInfo[] = (String [])sm.cctk.get(i);
//				
//				String tkName = (String)sm.zm.get(Integer.parseInt(tkInfo[0]));
//				String arrive = (String)tkInfo[2];
//				String leave = (String)tkInfo[3];
//				
//				System.out.println(tkName + "站 " + arrive + "到 " + leave + "发");
//			}

//			System.out.println(sm.getTrain("K100A"));
			
//			Vector trains = sm.findTrains("南通");
//			for(int i=0; i<trains.size(); i++) {
//				System.out.println(trains.get(i));
//			}
			
//			sm.buildTRF("C:\\trains\\合九线.cir", "C:\\trains\\trf_file\\合九线\\");
